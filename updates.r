...
update_F1 <- function(Xinput, Finput, Sinput, Ginput, phi, k){
  #' X: Input matrix
...
update_G <- function(Xinput, Finput, Sinput, Ginput, psi, k){
  ...
  #FS^T - calculate once to improve speed
  com_mat <- Finput %*% Sinput
  numerator_matrix <- t(Xinput) %*% com_mat
  denominator_matrix <- currentG %*% t(com_mat) %*% com_mat
