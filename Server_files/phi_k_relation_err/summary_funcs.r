...
all_table <- function(df,phi,filename, ndig=4,feat="Method", group=TRUE){
    ...
    for(i in 1:length(phi)){
        max_id <- max_index(df_overall[[as.character(phi[i])]])
        cond <- (1:length(df_overall[,1][[1]]))==max_id
        df_overall[[as.character(phi[i])]] <- text_spec(df_overall[[as.character(phi[i])]] , "latex", 
            bold = cond)
    }
...
#updated all_table
all_table2 <- function(df,k_vec,phi_vec, filename, ndig = 4,feat = "Method", group=TRUE){
    df <- mutate_at(df,c(feat, "Measure"), as.factor)
    df <- pivot_wider(df, names_from = k, values_from = c("Mean", "S.d."))
    #round to 4 digits
    df <- mutate_if(df, is.numeric, function(x) format(round(x, ndig), nsmall = ndig))
    #
    #add info in brackets
    mean_col_names <- paste0("Mean_",k_vec)
    sd_col_names <- paste0("S.d._",k_vec)
    #name of mean column
    for (i in 1:length(k_vec)){
        df[[as.character(k_vec[i])]] <-sd_brackets(df[[mean_col_names[i]]],df[[sd_col_names[i]]])
    }
    df_all <- subset(df, select = c("Phi", as.character(k_vec)))
    colnames(df_all) <- c(feat, as.character(k_vec))
    ...
    #subset only overall results and remove extras
    df_overall <- subset(df,select=c(feat,as.character(k_vec)))

    #make the largest values down a column bold
    for (i in 1:length(k_vec)){
        min_id <- min_index(df_overall[[as.character(k_vec[i])]])
        cond <- (1:length(df_overall[,1][[1]]))==min_id
        df_overall[[as.character(k_vec[i])]] <- text_spec(df_overall[[as.character(k_vec[i])]] , "latex", 
            bold = cond)
    }
    #make the largest values down a column bold
    for (i in 1:length(test_phi)){
        min_id <- min_index(df_overall[i,as.character(k_vec)])
        cond <- (1:length(k_vec))==min_id
        df_overall[i,as.character(k_vec)] <- as.list(text_spec(df_overall[i,as.character(k_vec)] , "latex", 
            color = ifelse(cond,"red","black")))
    }
    df_overall <- kbl(df_overall,booktabs=T,"latex",escape=FALSE)
    df_overall <- kable_styling(add_footnote(df_overall, c("Best performance in bold.")))
    #save
