    ...
    for (t in 1:nIter){
      ...
      for (v in 1:n_v){
        ...
        if (v != n_v){
            for (w in (v+1):n_v){
              if(phi[v,w]!=0){
                err[v] <- err[v] + phi[v,w]*norm((currentF[[v]] - currentF[[w]]), type = "F")
              }
              if(psi[v,w]!=0){
                err[v] <- err[v] + psi[v,w]*norm((currentG[[v]] - currentG[[w]]), type = "F")
              }
              if(xi[v,w]!=0){
                err[v] <- err[v] + xi[v,w]*norm((currentS[[v]] - currentS[[w]]), type = "F")
              }
            }
        } 
      }
