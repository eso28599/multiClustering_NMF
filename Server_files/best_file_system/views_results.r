...
measures <- c("CSR", "Rel", "Rec", "F_score", "BiS")
results[["Method"]] <- ifelse(results[["Method"]] == "res_nmtf_2",
                    "2", ifelse(results[["Method"]] == "res_nmtf_3",
                    "3", "4"))
...
#make and saves plot 
n_clusts <- length(row_start[1][[1]])
subhead <- TeX(paste0(n_clusts, " biclusters, ",
                 "$\\phi$", " = ", val, ", ",
                  n_repeats, " repetitions."))
plot_title <- "The effect of increasing the number of views on performance"
x_title <- "Number of views"
path_to_save <- paste0(path_to_sim_folder, "/increasing_views_plot.pdf")
suppressMessages(make_plot_line(results2, "Overall", plot_title,
                subhead,x_title,
                path_to_save))

path_to_save <- paste0(path_to_sim_folder, "/increasing_views_")
#produce tables

col_names <- c("No. of views", "CSR", "Relevance","Recovery", "F score", "BiS")
