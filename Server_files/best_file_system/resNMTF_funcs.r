...
##initialisation
init_mats <- function(X, KK, LL, sigma_I = 0.05){
...
clustering_res_NMTF <- function(Xinput, Foutput,
  ...
  if (sum(s_score) == 0){
    ...
    sil  <- ifelse(denom == 1, mean(s_score[s_score != 0]),
                 mean(s_score[s_score != 0]) - 2 * sd(s_score[s_score != 0]))
...
restMultiNMTF_run <- function(Xinput, Finput=NULL, Sinput=NULL, 
            ...
            nIter=1000, relErr="Sil", k_min=3, k_max =6, repeats = 5, index = 2, no_clusts = FALSE, 
             sample_rate = 0.8, n_stability = 5, stab_thres = 0.6, stability = TRUE){
  ...
  while (test == max_i) {
    ...
    if(relErr == "Sil"){
      ...
      err_list <- c(err_list, res_list[[new_l]][["Error"]])
