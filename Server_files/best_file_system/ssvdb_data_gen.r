...
true_rows <- matrix(0, 200, 4)
true_cols <-list(matrix(0, 1000, 4), matrix(0, 1000, 4))
U <- matrix(0, 200, 200)
full_list <- 1:200
biclustered <- c(250)
for(i in 1:4){
    non_zeros <- sample(full_list[-biclustered], 50)
...
#noise
sigma2 <- 0.2
noise1 <- mvrnorm(200, rep(0, 1000), sigma2*diag(1, 1000))
noise2 <- mvrnorm(200, rep(0, 1000), sigma2*diag(1, 1000))
#add noise
X1 <- abs(X1 + noise1)
X2 <- abs(X2 + noise2)
data <- list("v1"=X1, "v2"=X2)
path_to_save <- paste0(paste0(path_to_sim_folder, "/data/"),
                         batch_folder)

#save data
for(i in 1:4){
    data_mod <- list("v1"=X1, "v2"=numbers[i]*X2)
