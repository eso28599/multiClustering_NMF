...
factor_name <- "Sigma"
x_title <- "Sigma"
plot_title <- "The effect of increasing the level of noise"
file_names <- c(paste0("res_nmtf_", numbers),
    paste0("gfa_", numbers), 
    paste0("issvd_", numbers), paste0("nmtf_", numbers))
n <- length(numbers)
method_vec_res <- c(rep("ResNMTF", n), rep("GFA", n),
 rep("iSSVD", n), rep("NMTF", n))
factor_vec <- rep(numbers1, 4)
k_vec <- rep(4, 4*n)
phi_constant <- TRUE
n_col_plots <- 2
n_row_plots <- 2
