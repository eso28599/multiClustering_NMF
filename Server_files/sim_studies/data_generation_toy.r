...
multi_view <- function(row_dims, col_dims, row_start, row_end,  col_start, col_end, noise=10,row_same_shuffle=TRUE, col_same_shuffle=TRUE, seed=FALSE){
    ...
    if(any(col_same_shuffle)){
        new_col_ind <- sample((col_dims[col_same_shuffle])[1])
    }
    ...
    for(i in 1:n_views){
        ...
        if(!col_same_shuffle[i]){
            diff_col_ind <- sample(col_dims[i])
            data_i <- one_view_adv(row_dims[i], col_dims[i],
                 row_start[[i]], row_end[[i]],  col_start[[i]],
                  col_end[[i]], noise)
            X_trial[[i]] <- (data_i$view)[new_row_ind, diff_col_ind]
            true_row_clusterings[[i]] <- (data_i$truth_row)[new_row_ind, ]
            true_col_clusterings[[i]] <- (data_i$truth_col)[diff_col_ind, ]
        }else{
            data_i <- one_view_adv(row_dims[i], col_dims[i],
                 row_start[[i]], row_end[[i]],  col_start[[i]],
                  col_end[[i]], noise)
