...
make_plot_phi <- function(data_frame, cluster, plot_title, sub_title, filename){
...
all_table2 <- function(df,measures, filename, ndig = 4,feat = "Method", group=FALSE){
    ...
    #subset only overall results and remove extras
    df_overall <- subset(df,select=c(feat,"Choice", measures))
