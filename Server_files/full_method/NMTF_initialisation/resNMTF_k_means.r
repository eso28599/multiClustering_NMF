...
# Initialisation function based on k means
init_mats <- function(Xinput, KK, LL){
  ...
  for (v in 1:n_v){
    kmeans_col <- kmeans(x = Xinput[[v]], centers = LL[v])
    kmeans_row <- kmeans(x = t(Xinput[[v]]), centers = KK[v])
...
# Main function
restMultiNMTF_main <- function(Xinput, Finput=NULL, Sinput=NULL, Ginput=NULL, KK=NULL, LL=NULL, phi=NULL, xi=NULL, psi=NULL, nIter=NULL){
  ...
  return(list("Foutput" = Foutput, "Soutput" = Soutput,
              "Goutput" = Goutput, "Error" = error, "All_Error" = total_err,
