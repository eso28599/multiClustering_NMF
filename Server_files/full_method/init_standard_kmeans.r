...
#apply method
nmtf_results <- restMultiNMTF_run(Xinput = data_views, phi = phi, nIter = nIter)
...
error_nmtf_filename <- paste0(data_name, "/nmtf_errors.csv")
...
error_nmtf_filename <- paste0(data_name, "/nmtf_k_means_errors.csv")
openxlsx::write.xlsx(nmtf_results$row_clusters, file = row_nmtf_filename)
openxlsx::write.xlsx(nmtf_results$col_clusters, file = col_nmtf_filename)
write.csv(nmtf_results$Error, file = error_nmtf_filename)