...
evaluate_simulation <- function(row_clustering,col_clustering, true_row_clustering, true_col_clustering){
  ...
  for (i in 1:n_views){
    ...
    adjRandValue[2,i] <- adjustedRandIndex(col_clustering[[i]], true_col_clustering[[i]])
    nmiValue[1,i] <- NMI(as.vector(row_clustering[[i]]), as.vector(true_row_clustering[[i]]))
    nmiValue[2,i] <- NMI(as.vector(col_clustering[[i]]), as.vector(true_col_clustering[[i]]))
