...
for (i in 1:length(folder_names)){
    ...
    avgs_nmtf <- get_averages(avgs_nmtf, data_name, "results_nmtf", i)
    avgs_nmtf_k_means <- get_averages(avgs_nmtf_k_means, data_name, "results_k_means_nmtf",i)
}
...
for (i in 1:length(measures)){
    results <- rbind(results,
                    avg_over_repeats(avgs_nmtf,i,measures[i],"NMTF"),
                     avg_over_repeats(avgs_nmtf_k_means,i,measures[i],"NMTF - k means"))
}
...
for (cluster in averages){
    ...
    make_plot(results, cluster, paste0(cluster, " performance with different initialisations"),
                paste0(n_repeats, " repetitions"),
                path_to_save)
}
path_to_save <- paste0(path_to_sim_folder,"/diff_init_table_")
#produce tables
all_table(results,measures,path_to_save)
