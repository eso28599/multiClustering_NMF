...
get_averages <- function(list_avgs, data_name, filename, repeat_no){
...
avg_over_repeats <- function(avgs, i, measure, name){
...
make_plot_params <- function(data_frame, cluster, plot_title, sub_title, filename){
    p <- ggplot(subset(data_frame, Cluster==cluster), aes(x=Phi, y=Mean, group=Measure, color=Measure)) + 
...
all_table <- function(df,measures,filename, ndig=4){
    df <-mutate_at(df,c("Method", "Cluster", "Measure"), as.factor)
    ...
    for (i in 1:length(measures)){
        df[[measures[i]]] <-sd_brackets(df[[mean_col_names[i]]],df[[sd_col_names[i]]])
    }
    df_all <- subset(df,select=c("Cluster",measures))
    colnames(df_all) <- c("Method",measures)
    #collapse method rows
    #df_all <-kable_styling(kable(collapse_rows_df(df,Method),"latex"))
    group_tab <- kable_styling(kbl(df_all, booktabs = T,"latex"))
    #select groups
    methods <- unique(df$Method)
    for(i in 1:length(methods)){
        group_tab <- pack_rows(group_tab,methods[i], 3*i-2, 3*i)
    }
    #subset only overall results and remove extras
    df_overall <- subset(df,Cluster=="Overall",select=c("Method",measures))
    ...
    #df_overall <- kable_styling(kbl(df_overall,booktabs=T,"latex",escape=FALSE))
...
#updated all_table
all_table <- function(df,measures,filename, ndig=4,feat="Method"){
