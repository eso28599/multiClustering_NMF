# functions to implement biclustering extension of multi-view CoNMF - mulCoNMTF
# note; some functions from 'working_method_new_updates are needed' 
# Libraries
library(MASS)
library(eList)
library(MASS)
...
update_F_co <- function(Xinput, Finput, Sinput, Ginput, phi, k){
  #' X: Input matrix
...
update_G_co <- function(Xinput, Finput, Sinput, Ginput, k){
  #' X: Input matrix
  #' F: row-clustering 
  #' S: connection between row-clustering and column-clustering
...
update_S_co <- function(Xinput, Finput, Sinput, Ginput, k){
  #' X: Input matrix
  #' F: row-clustering
...
mulCoNMTF_algo <- function(Xinput, Finput, Sinput, Ginput, phi, nIter){
  #' X: Input matrix
  ...
  for (v in 1:n_v){
    # Update F
    currentF[[v]] <- update_F_co(Xinput = Xinput[[v]],
                              Finput = currentF,
                              Sinput = currentS[[v]],
                              Ginput = currentG[[v]],
                              phi = phi, k = v)
    ...
    # Update G
    currentG[[v]] <- update_G_co(Xinput = Xinput[[v]],
                              Finput = currentF[[v]],
                              Sinput = currentS[[v]],
                              Ginput = currentG, k = v)
    ...
    # Update S
    currentS[[v]] <- update_S_co(Xinput = Xinput[[v]],
                              Finput = currentF[[v]],
                              Sinput = currentS,
                              Ginput = currentG[[v]], k = v)
  }
  Foutput <- currentF
  Soutput <- currentS
  Goutput <- currentG
...
# Main function
mulCoNMTF_run <- function(Xinput, Finput=NULL, Sinput=NULL, Ginput=NULL, KK=NULL, LL=NULL, phi=NULL, nIter=NULL){
  ...
  if (is.null(nIter)){
    ...
    while (err_diff > 1.0e-4){
      ...
      new_parameters <- mulCoNMTF_algo(X = Xinput, 
                                           Finput = currentF, 
                                           Sinput = currentS,
                                           Ginput = currentG,
                                           phi = phi)
